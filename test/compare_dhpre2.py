"""
Compare dh_pre2 computed in main function vs kernel 1
"""
...
# Kernel to compute dh_pre2 (same as in kernel 1)
@triton.jit
def compute_dhpre2_kernel(
    x_ptr,
    h_pre_ptr,
    dh_in_ptr,
    dh_pre2_output_ptr,
    ...
    tl.store(dh_pre2_output_ptr + dh_pre2_offset, dh_pre2, mask=h_pre_mask)

# Allocate output
dh_pre2_kernel = torch.zeros(B, S, n, dtype=torch.float32, device=device)
...
with torch.cuda.device(device):
    compute_dhpre2_kernel[grid](
        x_ptr=x_dev,
        h_pre_ptr=h_pre_dev,
        dh_in_ptr=dh_in_dev,
        dh_pre2_output_ptr=dh_pre2_kernel,
        hc_eps=hc_eps,
        B=B, S=S, n=n, D=D,
        stride_x_b=S * n * D,
        stride_x_s=n * D,
...
# Compute dh_pre2 on CPU (same as main function)
x_fp = x.float()
...
print("Comparison of dh_pre2:")
print(f"  CPU (main function): {dh_pre2_gold[0, 0, :].numpy()}")
print(f"  GPU (kernel 1):      {dh_pre2_kernel[0, 0, :].cpu().numpy()}")
print(f"  Max error:  {torch.abs(dh_pre2_kernel.cpu() - dh_pre2_gold).max().item():.8f}")
print(f"  Mean error: {torch.abs(dh_pre2_kernel.cpu() - dh_pre2_gold).mean().item():.8f}")
print()

# Now compute dalpha_pre using both dh_pre2 values
h_mix_tmp = h_mix * inv_rms[:, :, None]
h_pre1_gold = h_mix_tmp[:, :, 0:n]

dalpha_pre_cpu = (dh_pre2_gold * h_pre1_gold).sum()
dalpha_pre_gpu = (dh_pre2_kernel.cpu() * h_pre1_gold).sum()

print("dalpha_pre comparison:")
print(f"  Using CPU dh_pre2: {dalpha_pre_cpu:.8f}")
print(f"  Using GPU dh_pre2: {dalpha_pre_gpu:.8f}")
print(f"  Difference:       {abs(dalpha_pre_gpu - dalpha_pre_cpu):.8f}")
