"""
直接测试kernel 1的dalpha计算，不经过其他kernel干扰
"""

import torch
import triton
import triton.language as tl
import sys
sys.path.insert(0, '.')

from src.forward.golden import mhc_forward_pre


@triton.jit
def test_dalpha_kernel(
    x_ptr,
    alpha_ptr,
    inv_rms_ptr,
    h_mix_ptr,
    h_pre_ptr,
    dh_in_ptr,
    dalpha_ptr,
    B, S, n, D, nD, out_features,
    stride_x_b, stride_x_s, stride_x_n, stride_x_d,
    stride_hin_b, stride_hin_s, stride_hin_d,
    stride_hpost_b, stride_hpost_s, stride_hpost_n,
    stride_hmix_b, stride_hmix_s, stride_hmix_out,
    norm_eps: tl.float32,
    hc_eps: tl.float32,
    BLOCK_SIZE_N: tl.constexpr,
    BLOCK_SIZE_K: tl.constexpr,
):
    """简化的dalpha计算kernel，只计算dalpha并输出"""
    pid = tl.program_id(axis=0)
    b_idx = pid // S
    s_idx = pid % S

    x_off_n = tl.arange(0, BLOCK_SIZE_N)
    x_mask = x_off_n < n

    # Load dh_in
    dh_in_off = tl.arange(0, BLOCK_SIZE_K)
    dh_in_mask = dh_in_off < D
    dh_in_offset = (b_idx * stride_hin_b + s_idx * stride_hin_s + dh_in_off * stride_hin_d)
    dh_in = tl.load(dh_in_ptr + dh_in_offset, mask=dh_in_mask, other=0.0)

    # Load x
    x_off_d = tl.arange(0, BLOCK_SIZE_K)
    x_mask2 = (x_off_n[:, None] < n) & (x_off_d[None, :] < D)
    x_offset = (b_idx * stride_x_b + s_idx * stride_x_s +
                 x_off_n[:, None] * stride_x_n +
                 x_off_d[None, :] * stride_x_d)
    x_block = tl.load(x_ptr + x_offset, mask=x_mask2, other=0.0).to(tl.float32)

    # Load h_pre
    h_pre_off = (b_idx * stride_hpost_b + s_idx * stride_hpost_s + x_off_n)
    h_pre = tl.load(h_pre_ptr + h_pre_off, mask=x_mask, other=0.0)

    # Load inv_rms
    inv_rms_off = b_idx * S + s_idx
    inv_rms = tl.load(inv_rms_ptr + inv_rms_off)

    # Load alpha
    a_pre = tl.load(alpha_ptr + 0)

    # Compute dh_pre
    dh_pre = tl.sum(x_block * dh_in[None, :], axis=1)

    # Compute dh_pre2
    s_pre2 = h_pre - hc_eps
    dh_pre2 = dh_pre * s_pre2 * (1.0 - s_pre2)

    # Compute dh_pre1
    dh_pre1 = a_pre * dh_pre2

    # Load h_pre1_hmix (这是h_mix[0:n] * inv_rms)
    h_pre1_hmix = tl.load(h_mix_ptr + (b_idx * S * out_features + s_idx * out_features + x_off_n),
                          mask=x_mask, other=0.0) * inv_rms

    # Compute dalpha_pre
    dalpha_pre_sum = tl.sum(dh_pre2 * h_pre1_hmix)

    # 直接存储到dalpha_ptr的第一个元素
    tl.atomic_add(dalpha_ptr + 0, dalpha_pre_sum)


def test_dalpha_isolated():
    """隔离测试dalpha计算"""
    print("=" * 70)
    print("隔离测试 dalpha_pre 计算")
    print("=" * 70)

    # Setup
    B, S, n, D = 2, 64, 4, 128
    device = 'cuda'
    hc_eps = 1e-6
    norm_eps = 1e-6
    nD = n * D
    out_features = n * n + 2 * n

    torch.manual_seed(42)
    x = torch.randn(B, S, n, D, dtype=torch.bfloat16, device=device)
    phi = torch.randn(n*n + 2*n, n*D, dtype=torch.float32, device=device)
    alpha = torch.tensor([1.1, 0.9, 1.05], dtype=torch.float32, device=device)
    bias = torch.randn(n*n + 2*n, dtype=torch.float32, device=device) * 0.1

    # Forward pass
    with torch.no_grad():
        h_in, h_post, h_res, inv_rms, h_mix, h_pre = mhc_forward_pre(
            x, phi, alpha, bias, outflag=True
        )

    # Gradients - USE SAME SEED AS FULL TEST
    torch.manual_seed(123)
    dh_in = torch.randn(B, S, D, dtype=torch.bfloat16, device=device)

    # Allocate output
    dalpha_tri = torch.zeros_like(alpha)

    # Ensure contiguous
    x = x.contiguous()
    alpha = alpha.contiguous()
    inv_rms = inv_rms.contiguous()
    h_mix = h_mix.contiguous()
    h_pre = h_pre.contiguous()
    dh_in = dh_in.contiguous()

    BLOCK_SIZE_N = triton.next_power_of_2(n)
    BLOCK_SIZE_K = triton.next_power_of_2(D)

    # Run test kernel
    grid = (B * S,)
    with torch.cuda.device(x.device.index if x.is_cuda else 0):
        test_dalpha_kernel[grid](
            x_ptr=x,
            alpha_ptr=alpha,
            inv_rms_ptr=inv_rms,
            h_mix_ptr=h_mix,
            h_pre_ptr=h_pre,
            dh_in_ptr=dh_in,
            dalpha_ptr=dalpha_tri,
            B=B, S=S, n=n, D=D, nD=nD, out_features=out_features,
            stride_x_b=S * n * D,
            stride_x_s=n * D,
            stride_x_n=D,
            stride_x_d=1,
            stride_hin_b=S * D,
            stride_hin_s=D,
            stride_hin_d=1,
            stride_hpost_b=S * n,
            stride_hpost_s=n,
            stride_hpost_n=1,
            stride_hmix_b=S * out_features,
            stride_hmix_s=out_features,
            stride_hmix_out=1,
            norm_eps=norm_eps,
            hc_eps=hc_eps,
            BLOCK_SIZE_N=BLOCK_SIZE_N,
            BLOCK_SIZE_K=BLOCK_SIZE_K,
        )

    # Compute golden reference
    a_pre = alpha[0]
    x_fp = x.float()
    h_in_fp_grad = dh_in.float()
    dh_pre_gold = (h_in_fp_grad.unsqueeze(2) * x_fp).sum(dim=-1)
    hc_eps_tensor = torch.tensor(hc_eps, device=x.device)
    s_pre2_gold = h_pre - hc_eps_tensor
    dh_pre2_gold = dh_pre_gold * s_pre2_gold * (1.0 - s_pre2_gold)
    h_mix_tmp = h_mix * inv_rms[:, :, None]
    h_pre1_gold = h_mix_tmp[:, :, 0:n]
    dalpha_pre_gold = (dh_pre2_gold * h_pre1_gold).sum()

    print("\n结果对比:")
    print(f"Golden dalpha_pre:  {dalpha_pre_gold:.8f}")
    print(f"Triton dalpha_pre:  {dalpha_tri[0]:.8f}")
    print(f"绝对误差:         {abs(dalpha_tri[0] - dalpha_pre_gold):.8f}")
    print(f"相对误差:         {abs(dalpha_tri[0] - dalpha_pre_gold) / abs(dalpha_pre_gold) * 100:.2f}%")

    if abs(dalpha_tri[0] - dalpha_pre_gold) < 0.001:
        print("\n✓ PASS: dalpha_pre计算正确！")
    else:
        print("\n✗ FAIL: dalpha_pre计算错误！")
        print("\n可能原因:")
        print("  1. h_mix_ptr指向错误的数据")
        print("  2. atomic_add的浮点精度问题")
        print("  3. 内存布局或stride问题")


if __name__ == "__main__":
    test_dalpha_isolated()
