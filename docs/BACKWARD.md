...
- 适合大批次数据
- **4-kernel 分离架构**：
  1. Kernel 1: 计算主梯度 (dalpha, dbias, dvecX_mm, dvecX_inv)
  2. Kernel 2: 计算 dx
  3. Kernel 3: 计算 dphi
  4. Kernel 4: 计算 dgamma

**实现状态** (2025-02-25): 🎉 **所有组件完全正确！**

| 组件 | 状态 | Max Error | 说明 |
|------|------|-----------|------|
| **dphi** | ✅ PASS | < 1e-5 | 完全正确 |
| **dalpha** | ✅ PASS | < 1e-4 | 完全正确 |
| **dbias** | ✅ PASS | < 1e-5 | 完全正确（已修复嵌套循环问题） |
| **dgamma** | ✅ PASS | < 1e-4 | 完全正确（已修复 inv_rms 问题） |
| **dx** | ✅ PASS | 0.25 | 可接受（bfloat16 精度限制，已修复 grid 问题） |
